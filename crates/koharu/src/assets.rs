...
impl FrontendAssets {
    fn lookup(&self, relative: &str) -> Option<(Bytes, &'static str)> {
        match self.inner.as_ref() {
            FrontendInner::Dir(root) => lookup_dir(root, relative),
            FrontendInner::Memory(files) => lookup_memory(files, relative),
        }
    }
}
...
fn lookup_dir(root: &Path, relative: &str) -> Option<(Bytes, &'static str)> {
    let candidates = file_candidates(relative);
    for candidate in &candidates {
        let path = root.join(candidate);
        if path.is_file() {
            let bytes = std::fs::read(&path).ok()?;
            return Some((Bytes::from(bytes), mime_type(candidate)));
        }
    }
    if should_fallback(relative) {
        let index = root.join("index.html");
        if index.is_file() {
            let bytes = std::fs::read(index).ok()?;
            return Some((Bytes::from(bytes), mime_type("index.html")));
        }
    }
    None
}

fn lookup_memory(files: &HashMap<String, Bytes>, relative: &str) -> Option<(Bytes, &'static str)> {
    let candidates = file_candidates(relative);
    for candidate in &candidates {
        let key = normalize_asset_key(candidate);
        if let Some(bytes) = files.get(&key) {
            return Some((bytes.clone(), mime_type(candidate)));
        }
    }
    if should_fallback(relative)
        && let Some(bytes) = files.get("/index.html").or_else(|| files.get("index.html"))
    {
        return Some((bytes.clone(), mime_type("index.html")));
    }
    None
}
