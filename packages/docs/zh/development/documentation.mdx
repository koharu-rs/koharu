...
在 Mintlify 控制台连接 `koharu-rs/koharu` 并选择生产分支。在 Git Settings 中启用 **docs.json is in a subdirectory**，输入不带结尾斜杠的 `/packages/docs`。在该路径设置完成前，仓库根目录的 `docs.json` 会指向 `packages/docs`，以便 PR 预览能找到配置。添加自定义域名 `koharu.rs`，按 Mintlify 显示的记录配置 DNS。
