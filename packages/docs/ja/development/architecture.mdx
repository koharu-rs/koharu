...
flowchart TB
  ...
  entry --> desktop["koharu-desktop"]
  bridge -->|"コマンドとチャンネル"| app
  app --> scene["koharu-scene"] --> storage["koharu-storage"]
  app --> pipeline["koharu-pipeline"] --> ml["koharu-ml"] --> runtime["ネイティブランタイム"]
  app --> translator["koharu-translator"]
  app --> agent["koharu-agent"]
  app --> desktop --> renderer["koharu-renderer"] --> rasterizer["koharu-rasterizer"]
  bridge --> canvas["koharu-canvas"] --> rasterizer
  renderer --> psd["koharu-psd"]
  rasterizer --> psd
```
...
独立した型付きチャンネルがプロジェクト、キャンバス、ジョブ、ダウンロード、設定、リソースの更新を運びます。UI は HTTP クライアントや汎用アプリイベントの封筒を使いません。
