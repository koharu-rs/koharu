...
export const commands = {
	...
	// OrcaRouter authentication. `connectOrcaRouter` returns the consent URL for
	// the user to open; the credential itself is issued and stored natively.
	connectOrcaRouter: () => __TAURI_INVOKE<OrcaRouterAuthorization>("connect_orca_router"),
	cancelOrcaRouter: () => __TAURI_INVOKE<null>("cancel_orca_router"),
	orcaRouterNeedsReauth: () => __TAURI_INVOKE<boolean>("orca_router_needs_reauth"),
	disconnectOrcaRouter: () => __TAURI_INVOKE<null>("disconnect_orca_router"),
...
export type OrcaRouterAuthorization = {
...
export type Provider = "local" | "atlas-cloud" | "openai" | "gemini" | "claude" | "grok" | "minimax" | "deepseek" | "openai-compatible" | "openrouter" | "orcarouter" | "lm-studio" | "deepl" | "google-cloud-translation" | "caiyun";

export type ProviderConfig = { provider: "local"; settings: LocalConfig } | { provider: "atlas-cloud"; settings: AtlasCloudConfig } | { provider: "openai"; settings: OpenAiConfig } | { provider: "gemini"; settings: GeminiConfig } | { provider: "claude"; settings: ClaudeConfig } | { provider: "grok"; settings: GrokConfig } | { provider: "minimax"; settings: MiniMaxConfig } | { provider: "deepseek"; settings: DeepSeekConfig } | { provider: "openai-compatible"; settings: OpenAiCompatibleConfig } | { provider: "openrouter"; settings: OpenRouterConfig } | { provider: "orcarouter"; settings: OrcaRouterConfig } | { provider: "lm-studio"; settings: LmStudioConfig } | { provider: "deepl"; settings: DeepLConfig } | { provider: "google-cloud-translation"; settings: GoogleCloudConfig } | { provider: "caiyun"; settings: CaiyunConfig };
